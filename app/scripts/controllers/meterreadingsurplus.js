...
angular.module('payPcApp')
	.controller('MeterreadingsurplusCtrl', function($scope, AppConfig,$http,BillService, $timeout,BuildingService,$rootScope,MeterService) {
		//树状图
		var vm = $scope.vm = {};
		vm.items = [{
		...
		var getList = function(pageIndex){ 	
			...
			if($scope.select.type ==88){ 
			 ...
			 MeterService.getLog($scope.select.BatchNo).success(function (response) {	
			...
			MeterService.getData({				
				...
				$rootScope.loading = false;
				if(response.Status == "SUCCESS"){
					$scope.dataList = response.Data.Meter;
		...
		$scope.edit = {
			...
			save:function(){
				...
				MeterService.updateData({
					 
					Flat:this.item.FlatId,				
					AllAmp:this.item.AllAmp,
					AllLastAmp:this.item.AllLastAmp,
					ChargeSum:this.item.ChargeSum,
					AllLastGiftAmp:this.item.AllLastGiftAmp,		
			...
			//if(item.BStatus !='已代扣'){
				swal({
					...
					closeOnConfirm: false
						...
						if (inputvalue) {
							$rootScope.loading = true;
						 MeterService.deleteData(item.ID).success(function (response) {
		...
		$scope.uploadFile = function(event){
			...
			var url = AppConfig.WEB_ROOT + "api/meter/import/" + $scope.select.peroid();
			swal({
				title: "请确认",
					...
					if (inputvalue) {
						...
						.success(function (response) {
							...
							if(response.Status == "SUCCESS"){
								...
								var msg = response.Message; /*"共从Excel文件中解析到" + response.Data.RecordCountInFile + '条记录，'
	...
	.factory('MeterService', ['$http', 'AppConfig',
		function($http, AppConfig) {
			var url = AppConfig.WEB_ROOT + 'api/Meter';
